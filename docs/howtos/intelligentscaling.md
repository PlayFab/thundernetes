...
Prediction is implemented using [linear regression](https://en.wikipedia.org/wiki/Linear_regression) and [Holt-Winters](https://en.wikipedia.org/wiki/Exponential_smoothing) methods. The algorithm takes the top value predicted by the algorithms and sets the `standingBy` accordingly. It is the user's responsibility to set the `max` with a proper value.
